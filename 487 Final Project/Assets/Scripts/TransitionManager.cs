...
public class TransitionManager : MonoBehaviour
{
	...
	private bool startedunZooming = false;
	private bool zoomed = false;
	private bool unzoomed = true;
	...
	void Update () 
	{
		...
		if (mPlaying)
		{
			...
			{
				if (!startedZooming && !zoomed && unzoomed) {
					InvokeRepeating ("ZoomCamera", .5f, .1f);
				}
				if (!startedunZooming && !unzoomed && zoomed) {
					InvokeRepeating ("UnZoomCamera", .3f, .1f);
				}
	...
	void ZoomCamera() {
		...
		Vector3 temp = ARcam.GetComponent<Transform> ().localScale;
		if (startedZooming) {
			if (!zoomed) {
				temp.z -= .003f * temp.z;
				ARcam.GetComponent<Transform> ().localScale = temp;
				if (temp.z <= .5) {
